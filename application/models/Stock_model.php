...
class Stock_model extends CI_Model {
	...
	function invoice_submit($invNO,$mainDate,$mainDeliDate,$net_total,$pro_code,$pro_name,$pro_id,$qty,$price,$net,$minvoice)
	{
		...
		$query=$this->db->query("INSERT INTO purchase_invoice VALUES('','$minvoice','','','$mainDate','','','$net_total','','$payment','','','','','')");
		...
		for($i=0;$i<$proInv;$i++)
		{
			if(($qty[$i]!='' && $qty[$i]>0))
			{
				...
				$query=$this->db->query("INSERT INTO purchase_invoice_details VALUES('','$inv_id','$minvoice','$pro_name[$i]','$pro_code[$i]','$pro_id[$i]','','','$qty[$i]','','','','$price[$i]','$net[$i]','','','$mainDate','','','','')");
