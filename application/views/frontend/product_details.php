					...
					<div class="pb-right-column prod_details col-xs-12 col-sm-6">
						...
						<div class="product_attributes">
							<div class="col-sm-12" style="margin:0; padding:0">
								...
								<div class="col-sm-4" style="border-left: 1px dotted black; display:none" id="showbonusid">
									<h3 style="color:#FF0000;">Bonus Item</h3>
									<p id="bonusname">Karchup-61</p>
									<input type="hidden" value="" id="bonusid" name="bonusid" />
									<!-- <img src="http://localhost/bargainnshop/uploads/images/product/main_img/thumnail/Bargainnshop_1544072843_thumb.jpg" class="img-responsive"> -->
									<img id="bonusimage" src="" class="img-responsive" style="height:50px">
									<p style="color:#009900">Price BDT <span id="bonusprice">1500</span> Tk</p>
								</div>
							</div>
							...
							<div class="col-md-12">
								...
								<div class="carousel slide" data-ride="carousel" data-type="multi" data-interval="3000" id="myCarousel">
									...
									$bounusitemprice = $ptotalprice - $discount;
									$bonusarray = $this->db->query("SELECT * FROM bonusrange WHERE To_pro_amount > {$bounusitemprice} AND {$bounusitemprice} <= To_pro_amount limit 1");
									$bonusdata = $bonusarray->row();
									$bonusamount = $bonusdata->bonus_amount;


									$productdata = $this->db->query("SELECT * FROM product WHERE price = {$bonusamount} ");
										// $this->db->where('price >=', $bonusamount);
										// $this->db->where('price <=', $bonusamount);
										// $productdata =  $this->db->get('product');
									// print "<pre>";
									// print_r($productdata->result());
									// print "<pre>";
									// print_r($bonusarray->result());
											if($productdata) {
											foreach($productdata->result() as $viewdata):
											// 	print "<pre>";
											// print_r($bonusgallery);
											// die;
									//	if(count($bonusarray->result())){
											
											
										?>
										<div class="item active">
											<div class="col-md-2 col-sm-6 col-xs-12 carousal-size">
												<div onclick='getbounsData("<?php echo $viewdata->product_id; ?>","<?php echo $viewdata->product_name; ?>","<?php echo $viewdata->price; ?>","<?php echo $viewdata->thumb; ?>")'
												 style="cursour:pointer">
													<img src="<?php echo base_url(); ?>uploads/images/product/main_img/thumnail/<?php echo $viewdata->thumb; ?>"
									...
									<a class="left carousel-control" href="#myCarousel" data-slide="prev"> <i class="fa fa-chevron-left"
										 aria-hidden="true"></i></a>
									<a class="right carousel-control" href="#myCarousel" data-slide="next"> <i class="fa fa-chevron-right"
										 aria-hidden="true"></i></a>
								</div>
								<script>
									function getbounsData(id,name,price,image){
										 //alert(name);
										 $("#showbonusid").show();
										 url = "<?php echo  base_url('uploads/images/product/main_img/thumnail/'); ?>";
										$("#bonusid").attr('value',id);
										$("#bonusname").html(name);
										$("#bonusimage").attr('src',url+image);
										$("#bonusprice").html(price);
									 }
									</script>
								<script>
				...
				<div class="page-product-box">
					...
					<div id="related-slidertab" class="owl-carousel">
						<?php 
							...
							foreach($relatednewproduct->result() as $gallery):
