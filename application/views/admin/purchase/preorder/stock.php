...
function loadContentHistory(pid)
{
	$.ajax({
            type: "POST",
            url: "<?php echo base_url();?>administration/pre_inventory_history",
            data: ({'pro_id' : pid}),
                                ...
                                <div class="container">
                                  <table class="table table-striped" width="100%">
									...
									?>
                                      ...
                                      <td align="left"> 
                                         	<?php 
											$stock=$this->db->query("SELECT sum(pro_qty) as qty FROM pre_stock WHERE pro_id='$product_id'");
											if($stock->num_rows() > 0){
												$drwos = $stock->row();
												$stockQty = $drwos->qty;
											...
											$returnpro=$this->db->query("SELECT sum(pro_qty) as qty FROM pre_return_product WHERE pro_id='".$product_id."'");
                                    ...
                                    <!--<a  href="<?php echo base_url('administration/pre_inventory_history/'.$product_id);?>" style="text-decoration:none;">
...
<?php echo form_open('administration/pre_stock_update');?>
