	...
	function feature_registration()
	{
		...
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			...
			if($this->form_validation->run() != false){
				...
				$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
				redirect('administration/feature_list', 'refresh');
			}
			else{
	...
	function class_registration()
	{
		...
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			...
			if($this->upload->do_upload('appIcon')){
				...
				$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
				redirect('administration/class_list', 'refresh');
			}
			else{
...
/////////////////////// advertisement ////////////////////////////////	 
	...
	function advertisement_registration()
	{
		...
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			...
			if($this->form_validation->run() != false){
				...
				$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
				redirect('administration/advertisement_list', 'refresh');
			}
			else{
	...
	function events_registration()
	{
		...
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			...
			$impval=implode('-',$expval);
				...
				$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
				redirect('administration/events_list', 'refresh');
			}
			else{
			...
			$save['active']	    = $this->input->post('status');
				...
				class ".$urlname." extends CI_Controller { ";
					...
					//$password=$this->input->post('password');
						...
						$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
						redirect('administration/boutique_list', 'refresh');
					//}
				}
			}
			else{
	...
	function agent_registration()
	{
		...
		if($this->form_validation->run() != false){
			...
			$save['passwordHints']	    = $this->input->post('password');
			  ...
			  $this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
			  redirect('administration/agent_list', 'refresh');
			}
			else{
	...
	function ajaxCategory()
	{
		if($this->input->get('class_id')!=""){
			...
			$url="'".base_url()."administration/ajaxCategory?cat_id='+this.value+''";
	...
	function photogallery_registration()
	{
		...
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			...
			$this->upload->initialize($config);
				...
				$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
				redirect('administration/photogallery_list', 'refresh');
			}
			else{
...
/*	
	...
	if(!$this->session->userdata('AdminAccessMail')) redirect("administration");
		...
		redirect('administration/inventory', '');
	}
	...
	function membership()
	{
		...
		$config['base_url'] = base_url('administration/membership');
        $config["per_page"] = 20;
		...
		$mid = $this->input->post('mid');
				...
				if($this->form_validation->run() != false){
					...
					if($query){
						...
						redirect('administration/membership');
					}
				}
				else{
					if(isset($mid) && $mid!=''){
	...
	public function membershipRenew(){
		...
		$data['title'] = 'Membership';
		$data['supplier'] = $this->Membership_model->getTable('supplier','	username','asc');
		$config['base_url'] = base_url('administration/membershipRenew/');
        $config["per_page"] = 30;
		...
		if(isset($url) && $url!=''){
			if($this->input->post('save')){
				...
				if($this->form_validation->run() != false){
					...
					if($query){
						$this->session->set_flashdata('successMsg', '<p class="alert alert-success">Successfully Member Created</p>');
						redirect('administration/membershipRenew');
					}
				}
			}
			if(isset($url) && $url=='new'){
